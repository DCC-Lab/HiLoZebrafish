...
void loop() {
  ...
  if (scramblerVal == HIGH && state == true) {
    // Pour les bonnes images
    if (image > 0 && image < totalNumberOfImages) {
      Serial.println(image);
      Serial.println(positionInZ);
    ...
    // Pour l'image tampon
      ...
      Serial.println(image);
      Serial.println(positionInZ);
      positionInZ = 0; // On reste a z=0
      image += 1; // On incrémente une image
    }
    // Pour la dernière image
    else if (image == totalNumberOfImages) {
      Serial.println(image);
      Serial.println(positionInZ);
      positionInZ = 0; // Retourner au plan de départ, à current = 0 mA
      image = 0; // Remettre l'image tampon
    }
  ...
  if (scramblerVal == LOW && state == false) {
    state = true; 
  }
